...
Action::add('admin_header','NewsAdmin::header', 9);
    
class NewsAdmin extends Backend {
    
    public static function header() {
        if (isset(Plugin::$plugins['markitup'])) {
            if (Request::get('id') == 'news') {
                echo "<script>$(document).ready(function(){mySettings.markupSet.push({name:'Cut', replaceWith:'{cut}', className:'news-cut' });});</script>";
            }
        }
    }
                        ...
                        if (Request::post('edit_news') || Request::post('edit_news_and_exit')) {
                            if (Security::check(Request::post('csrf'))) {
                                ...
                                if (count($errors) == 0) {
                                    ...
                                    if (Request::post('edit_news_and_exit')) Request::redirect('index.php?id=news');                      
                                    else Request::redirect('index.php?id=news&action=edit&news_id='.$id); 
                                }
                            }
                        }
                    ...
                    if (Request::post('add_news') || Request::post('add_news_and_exit')) {
                        if (Security::check(Request::post('csrf'))) {
                            ...
                            if (count($errors) == 0) {
                                ...
                                if (Request::post('add_news_and_exit')) Request::redirect('index.php?id=news');                                       
                                else Request::redirect('index.php?id=news&action=edit&news_id='.$last_id); 
                            }
                        } else { die('csrf detected!'); }
                    }
